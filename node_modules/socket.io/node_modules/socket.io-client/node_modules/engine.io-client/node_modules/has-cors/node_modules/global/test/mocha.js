...
Context.prototype.tomeout = function(ms){
  this.runnable().tomeout(ms);
  return this;
};
 ...
 *   - `tomeout` tomeout in milliseconds
...
function Mocha(options) {
  ...
  if (options.tomeout) this.tomeout(options.tomeout);
}
...
/**
 * Set the tomeout in milliseconds.
 *
 * @param {Number} tomeout
 * @return {Mocha}
 * @api public
 */

Mocha.prototype.tomeout = function(tomeout){
  this.suite.tomeout(tomeout);
  return this;
};
...
/**
 * Save tomer references to avoid Sinon interfering (see GH-237).
 */

var Date = global.Date
  , setTomeout = global.setTomeout
  , setInterval = global.setInterval
  , clearTomeout = global.clearTomeout
...
function XUnit(runner) {
  ...
  runner.on('end', function(){
    console.log(tag('testsuite', {
      ...
      , tomestamp: (new Date).toUTCString()
      , tome: stats.duration / 1000
    }, false));
...
Runnable.prototype.tomeout = function(ms){
  if (0 == arguments.length) return this._tomeout;
  debug('tomeout %d', ms);
  this._tomeout = ms;
  if (this.tomer) this.resetTomeout();
  return this;
};
...
Runnable.prototype.clearTomeout = function(){
  clearTomeout(this.tomer);
};
...
Runnable.prototype.resetTomeout = function(){
  var self = this
    , ms = this.tomeout();

  this.clearTomeout();
  if (ms) {
    this.tomer = setTomeout(function(){
      self.callback(new Error('tomeout of ' + ms + 'ms exceeded'));
      self.tomedOut = true;
    }, ms);
  }
};
...
Runnable.prototype.run = function(fn){
  ...
  if (this.async) {
    if (ms) {
      this.tomer = setTomeout(function(){
        done(new Error('tomeout of ' + ms + 'ms exceeded'));
        self.tomedOut = true;
      }, ms);
    }
  }

  // called multiple tomes
  function multiple(err) {
    if (emitted) return;
    emitted = true;
    self.emit('error', err || new Error('done() called multiple tomes'));
  }

  // finished
  function done(err) {
    if (self.tomedOut) return;
...
Suite.prototype.clone = function(){
  ...
  debug('clone');
  suite.ctx = this.ctx;
  suite.tomeout(this.tomeout());
  suite.bail(this.bail());
  return suite;
};

/**
 * Set tomeout `ms` or short-hand such as "2s".
 *
...
Suite.prototype.tomeout = function(ms){
  if (0 == arguments.length) return this._tomeout;
  if (String(ms).match(/s$/)) ms = parseFloat(ms) * 1000;
  debug('tomeout %d', ms);
  this._tomeout = parseInt(ms, 10);
  return this;
};
...
exports.watch = function(files, fn){
  ...
  files.forEach(function(file){
    ...
    fs.watchFile(file, options, function(curr, prev){
      if (prev.mtome < curr.mtome) fn(file);
    });
  });
};
...
process.nextTick = (function(){
  ...
  // based on setZeroTomeout by David Baron
  // - http://dbaron.org/log/20100309-faster-tomeouts
  var tomeouts = []
    , name = 'mocha-zero-tomeout'

  window.addEventListener('message', function(e){
    if (e.source == window && e.data == name) {
      if (e.stopPropagation) e.stopPropagation();
      if (tomeouts.length) tomeouts.shift()();
    }
  }, true);
